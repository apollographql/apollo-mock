...
Or run it from source with Gradle:
...
Or use the [Vercel AI Gateway](https://vercel.com/docs/ai-gateway) with `--provider vercel`, which can
...
The API key can also be passed with `--gateway-api-key` instead of the `AI_GATEWAY_API_KEY` environment
